	...
	while (true) {
		...
		* message is found in the queue */
		if (mxt_is_message_pending(&device)) {
			...
			xQueueSend( xQueueTouch, &touch, 0);           /* send mensage to queue */
		}
		vTaskDelay(100);
	}
}
...
void task_lcd(void){
	...
	while (true) {
		...
		if (xQueueReceive( xQueueADC, &(adc), ( TickType_t )  100 / portTICK_PERIOD_MS)) {
			...
			font_draw_text(&arial_72, b, ILI9488_LCD_WIDTH/2 - 40*2, ILI9488_LCD_HEIGHT/2 - 84/2, 2);

			if (adc.value <= 1000) {
				ili9488_set_foreground_color(COLOR_CONVERT(COLOR_RED));
				ili9488_draw_filled_rectangle(ILI9488_LCD_WIDTH/2 - 30, 60, ILI9488_LCD_WIDTH/2 -20, 100);
				ili9488_set_foreground_color(COLOR_CONVERT(COLOR_WHITE));
				ili9488_draw_filled_rectangle(ILI9488_LCD_WIDTH/2 - 15, 50, ILI9488_LCD_WIDTH/2 - 5, 100);
				ili9488_draw_filled_rectangle(ILI9488_LCD_WIDTH/2, 40, ILI9488_LCD_WIDTH/2 + 10, 100);
				ili9488_draw_filled_rectangle(ILI9488_LCD_WIDTH/2 + 15, 30, ILI9488_LCD_WIDTH/2 + 25, 100);
				ili9488_draw_filled_rectangle(ILI9488_LCD_WIDTH/2 + 30, 20, ILI9488_LCD_WIDTH/2 + 40, 100);
			} 
			else if (adc.value > 1000 && adc.value < 3000 ) {
...
void task_adc(void){
	...
	while(1){
		if( xSemaphoreTake(xSemaphoreADC, ( TickType_t ) 500) == pdTRUE ){
